...
public class UnidadeMedidaAction extends DispatchAction {
    ...
    public ActionForward adicionar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {        
            ...
            List<String> mensagens = new ArrayList<String>();
            mensagens.add("Ocorreu um erro ao tentar inserir a unidade de medida, verifique se já está cadastrada.");
            request.getSession().setAttribute(MensagemTag.LISTA_MENSAGENS, mensagens);
        }
        try {
    ...
    public ActionForward alterar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {        
            ...
            List<String> mensagens = new ArrayList<String>();
            mensagens.add("Ocorreu um erro ao tentar alterar a unidade de medida, verifique se já está cadastrada.");
            request.getSession().setAttribute(MensagemTag.LISTA_MENSAGENS, mensagens);
        }        
        request.setAttribute("acao", "UnidadeMedida.do");
        return mapping.findForward("redirect");
        ...
        try {            
            ...
            for (int i = 0; i < id.length; i++) {
                ...
                try {
                    ...
                    HibernateUtil.rollbackTransaction();
                    ex.printStackTrace();
                    erros.add("A unidade de medida \"" + unidadeMedida.getDescricao() + "\" está associada. Não pode ser excluída!");
